...
class EMMNGridSearch:
    def __init__(self, config_manager):
        ...
        self.results = []
        self.experiment_id = (
            f"emmn_grid_search_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
    ...
    def define_parameter_grid(self):
        ...
        grid = {
            ...
            "unsir_lr": [
                ...
                0.15,
                0.2,
            ],  # UNSIR learning rate (fixed in original)
    ...
    def UNSIR_noise_train(
        self, noise, model, forget_class_label, num_epochs, noise_batch_size, lr=0.1
    ):
        """
    ...
    def emmn_noise_train(
        self, noise, model, target_class_label, num_epochs, noise_batch_size, lr=0.1
    ):
        """
    ...
    def fit(self, epochs, lr, model, train_loader, opt_func=torch.optim.Adam):
        ...
        for epoch in range(epochs):
            ...
            if len(train_losses) > 0:
                ...
                if epoch % 1 == 0:
                    print(
                        f"Finetune Epoch {epoch+1}/{epochs}: Train Loss: {avg_train_loss:.4f}"
                    )
    ...
    def run_basic_emmn_unlearning(self, model, params):
        ...
        try:
            ...
            noise = UNSIR_noise(
                noise_batch_size, num_channels, img_shape, img_shape
            ).to(self.device)
            ...
            retain_classes = [
                i for i in range(self.num_classes) if i != self.forget_class
            ]
            retain_noises = {
                i_: UNSIR_noise(
                    noise_batch_size, num_channels, img_shape, img_shape
            ...
            for i_, n_ in retain_noises.items():
                retain_noises[i_] = self.emmn_noise_train(
                    n_, model, i_, emmn_epochs, noise_batch_size, lr=emmn_lr
                )
            ...
            noisy_loader = self.emmc_create_noisy_loader(
                noises, batch_size=noise_batch_size, num_noise_batches=num_noise_batches
            )
    ...
    def run_single_experiment(self, params, experiment_idx, total_experiments):
        ...
        try:
            ...
            print(
                f"  Forget Accuracy: {forget_acc:.4f}"
                if forget_acc != "N/A"
                else f"  Forget Accuracy: {forget_acc}"
            )
            print(
                f"  Retain Accuracy: {retain_acc:.4f}"
                if retain_acc != "N/A"
                else f"  Retain Accuracy: {retain_acc}"
            )
    ...
    def _create_readable_report(self, summary):
        ...
        with open(report_file, "w") as f:
            ...
            for category, experiment in summary["best_experiments"].items():
                ...
                f.write(
                    f"  Finetune Epochs: {experiment.get('finetune_epochs', 'N/A')}\n"
                )
                f.write(
                    f"  Noise Batch Size: {experiment.get('noise_batch_size', 'N/A')}\n"
                )
                f.write(
