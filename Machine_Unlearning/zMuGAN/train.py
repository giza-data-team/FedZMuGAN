...
import numpy as np
import torch
from Machine_Unlearning.zMuGAN.utils import sample_noises, sample_labels, visualize_images
...
def train(
    ...
    csv_file = os.path.join(logs_path, f"{dataset_name}_{model_name}_training_log{index_suffix}.csv")
    print(f"CSV file for logging: {csv_file}")

    with open(csv_file, mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["Epoch", "Cls Loss", "Dec Loss", "Div Loss", "Total Loss", "Accuracy"])
        ...
        for epoch in range(epochs):
            ...
            for batch_idx in range(num_batches):
                ...
                labels = sample_labels(batch_size, num_classes, dist='onehot').to(device)
                ...
                print(f"Calculated losses: Cls={loss1.item()}, Dec={loss2.item()}, Div={loss3.item()}, Total={loss.item()}")
            ...
            writer.writerow([epoch+1, f"{loss[0]:.4f}", f"{loss[1]:.4f}", f"{loss[2]:.4f}", f"{loss[3]:.4f}", f"{accuracy:.2f}"])
            file.flush()
            print("Logged epoch results to CSV")

            # Save sample images every 5 epochs
            if (epoch+1) % 5 == 0:
                sample_image_dir = os.path.join(output_dir, "samples", dataset_name, model_name + index_suffix)
                os.makedirs(sample_image_dir, exist_ok=True)

                sample_image_path = os.path.join(sample_image_dir, f"sample_epoch_{epoch+1}.png")
