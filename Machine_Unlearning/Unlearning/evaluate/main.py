...
from Machine_Unlearning.Unlearning.metrics.anamnesis_index import (
    anamnesis_index
)
...
class UnlearningEvaluator:
    def __init__(self):
        ...
        os.makedirs(self.unlearning_configs["SCRATCH_MODELS_PATH_UNLEARN"], exist_ok=True)
        ...
        self.unlearned_model = load_model(
    ...
    def _load_configurations(self):
        ...
        self.learning_rate = self.config_manager.get_lrs_original()[self.dataset_configs["DATASET_NAME"]][self.model_configs["MODEL_NAME"]]
        self.unlearning_configs = self.config_manager.get_unlearning_configs()
        self.specific_data_path = os.path.join(self.dataset_configs["DATASET_PATH"], self.dataset_configs["DATASET_NAME"])
    ...
    def _prepare_dataloaders(self):
        ...
        (
            ...
            self.dataset, self.unlearning_configs["BATCH_SIZE_AIN"], self.unlearning_configs["FORGET_CLASS"], dataset_type="validate"
        )
        ...
        (
            ...
            self.dataset, self.model_configs["BATCH_SIZE_ORIGINAL"], self.unlearning_configs["FORGET_CLASS"], dataset_type="train"
        )
        (
            self.forget_validate_loader,
            self.retain_validate_loader,
        ) = prepare_forget_retain_dataloaders(
            self.dataset, self.model_configs["BATCH_SIZE_ORIGINAL"], self.unlearning_configs["FORGET_CLASS"], dataset_type="validate"
        )
        (
            self.forget_test_loader,
            self.retain_test_loader,
        ) = prepare_forget_retain_dataloaders(
            self.dataset, self.model_configs["BATCH_SIZE_ORIGINAL"], self.unlearning_configs["FORGET_CLASS"], dataset_type="test"
        )
    ...
    def evaluate_unlearned_model(self, original_accuracy):
        _, unlearned_forget_accuracy = test(self.unlearned_model, self.forget_test_loader, self.device)
        _, unlearned_retain_accuracy = test(self.unlearned_model, self.retain_test_loader, self.device)
