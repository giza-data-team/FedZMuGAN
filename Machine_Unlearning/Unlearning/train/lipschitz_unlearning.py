...
class LipschitzUnlearning:
    def __init__(self, model, device, forget_dataloader, opt_func=torch.optim.Adam, 
                 learning_rate=0.001, noise_std=0.1):
        self.model = model
        self.device = device
        ...
        logger.info(f"Initializing Lipschitz Unlearning with lr={learning_rate}, noise_std={noise_std}")
    ...
    def lipschitz_unlearn(self):
        ...
        for batch_idx, sample in enumerate(self.forget_dataloader):
            ...
            if batch_idx % 10 == 0:
                logger.info(f"Batch {batch_idx}/{len(self.forget_dataloader)}, Loss: {loss.item():.6f}")
