...
class EMMNUnlearning:
    ...
    def UNSIR_noise_train(
        self, noise, model, forget_class_label, num_epochs, noise_batch_size
    ):
        """
    ...
    def emmn_noise_train(
        self, noise, model, target_class_label, num_epochs, noise_batch_size
    ):
        """
    ...
    def emmn_fit(self):
        ...
        logger.info(
            f"Using image shape: {img_shape}x{img_shape}, Channels: {num_channels}"
        )
        ...
        noise = UNSIR_noise(noise_batch_size, num_channels, img_shape, img_shape).to(
            self.device
        )
        ...
        retain_noises = {
            i_: UNSIR_noise(noise_batch_size, num_channels, img_shape, img_shape).to(
                self.device
            )
        ...
        for i_, n_ in retain_noises.items():
            retain_noises[i_] = self.emmn_noise_train(
                n_, model, i_, 400, noise_batch_size
            )
