...
from Machine_Unlearning.Unlearning.metrics.dataset import (
...
def __main__():
    ...
    forget_test_loader, retain_test_loader = prepare_forget_retain_dataloaders(
        dataset=dataset, batch_size=64, forget_class=3, dataset_type="test"
    )
    device = "cuda" if torch.cuda.is_available() else "cpu"

    # Load the model
    state_dict = torch.load(original_model_path)
    model = ModelFactory.create_model()  # Get the model instance
