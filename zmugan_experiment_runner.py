...
class ExperimentRunner:
    ...
    def update_env_file(self, lr_unlearn, batch_size_unlearn, samples_per_class):
        ...
        try:
            ...
            for line in lines:
                line = line.strip()
                if line.startswith('LR_UNLEARN'):
                    new_lines.append(f'LR_UNLEARN = {lr_unlearn}')
                elif line.startswith('BATCH_SIZE_UNLEARN'):
                    new_lines.append(f'BATCH_SIZE_UNLEARN = {batch_size_unlearn}')
                elif line.startswith('SAMPLES_PER_CLASS'):
                    new_lines.append(f'SAMPLES_PER_CLASS = {samples_per_class}')
                else:
            ...
            logger.info(f"Updated values: LR_UNLEARN={lr_unlearn}, BATCH_SIZE_UNLEARN={batch_size_unlearn}, SAMPLES_PER_CLASS={samples_per_class}")
    ...
    def run_experiment(self, lr_unlearn, batch_size_unlearn, samples_per_class):
        """Run a single experiment with given parameters"""
        logger.info(f"Running experiment with LR_UNLEARN={lr_unlearn}, "
                   f"BATCH_SIZE_UNLEARN={batch_size_unlearn}, "
                   f"SAMPLES_PER_CLASS={samples_per_class}")
        
        # Update .env file with new parameters
        self.update_env_file(lr_unlearn, batch_size_unlearn, samples_per_class)
        ...
        results_path = os.path.join("server_results", f"server_{model_name}_{n_clients}_{dataset_name}.csv")
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                if "learning and unlearning completed" in result.stderr:
                    logger.info("Experiment completed successfully (unlearning phase finished as expected)")
                else:
                ...
                with open(results_path, 'r') as f:
                    ...
                    if rows:
                        ...
                        metrics = {
                            ...
                            'anamnesis_index': last_row.get('avg_anamnesis_index', None),
                            'samples_per_class': samples_per_class,
                            'batch_size_unlearn': batch_size_unlearn,
                            'original_forget_accuracy': last_row.get('avg_original_forget_acc', None),
                            'original_retain_accuracy': last_row.get('avg_original_retain_acc', None)
                        }
                        
                        self.results.append(metrics)
                        logger.info(f"Added metrics to results list. Current results count: {len(self.results)}")
...
def main():
    ...
    samples_per_class = [ 25, 50, 100, 200, 300, 400, 500, 1000]
    ...
    for lr in learning_rates:
        for batch_size in batch_sizes:
            for samples in samples_per_class:
                runner.run_experiment(lr, batch_size, samples)
